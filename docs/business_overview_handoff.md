...
- 株たん由来の短い会社概要（`stocks.business_summary`）は、それ自体に価値があるため残す。
  AI整理文で置き換えるのではなく、**短く立ち位置を掴む文章**と
  **有報ベースで事業構造を理解する文章**として役割を分ける。
...
SELECT s.code, s.name, s.business_summary
FROM stocks s
...
CREATE TABLE IF NOT EXISTS business_overviews (
    ...
    overview          TEXT,         -- 整理文（である調・5〜8文・500〜800字）
...
3. **既存の短文概要**（あれば）: `stocks.business_summary`
   - 株たん由来の短文はそのままサイトに残す前提。
   - AI整理文に丸写ししない。
   - ただし「何の会社か」「業界内の立ち位置」「親会社・グループ関係」
     「意外な周辺事業」など、事業理解に役立つ観点の有無を確認する補助材料にする。
   - `business_summary` にしかない業界順位・競合比較・親子関係を本文に入れる場合は、
     断定しすぎず、可能なら `points_json.notes` に「短文概要由来」と分かる形で残す。
...
```text
有価証券報告書の「事業の内容」と、セグメント別売上構成比（公式数値）、
既存の短文概要（あれば）をもとに、企業の事業内容を5〜8文・全体500〜800字で整理してください。

- 文体は「である調」
- 冗長な表現、IR文書特有の長い修飾、関係会社名の羅列は避ける
- 主な事業、商材・サービス、顧客・販売先、特徴を読み取れる範囲で整理する
- 単なる製品列挙で終わらせず、投資家が最初に知りたい
  「何で稼ぐ会社か」「主力事業は何か」「どの顧客・業界向けか」
  「他の事業として何を持つか」が分かる文章にする
- 業界内ポジション、親会社・グループ関係、意外な周辺事業、強み・特徴が
  有報本文・セグメント数値・既存短文概要から読み取れる場合は簡潔に入れる
- セグメント別売上構成比が提供されている場合は整合させ、主力事業には
  「売上の約◯割」の形で規模感を添える
- 有報本文・提供数値・既存短文概要から直接読み取れない強み・弱み・競合比較は断定しない
  （「特徴である」「主力である」「短文概要では〜とされる」程度の表現に留める）
- 有報原文・提供数値・既存短文概要に無い固有名詞・数値を創作しない
- 仕入れ先・販売先が具体名で書かれていない場合は、顧客層・販売チャネルとして整理する
```

### 文章設計の考え方

AI整理文は、株たん由来の短文概要とは役割を分ける。株たん短文は
「一瞬で何の会社かを掴む」ために残し、AI整理文は「もう一段詳しく事業構造を理解する」
ための文章にする。

悪い方向は、有報の製品名をただ並べるだけの文章である。これは正確でも、
投資家にとって「結局、何が主力で、どこが特徴なのか」が残りにくい。
良い方向は、以下を自然な文章に織り込むことである。

- 主要事業の一言要約
- 主な商材・サービス
- 顧客業界・販売チャネル
- 業界内ポジションやグループ関係（根拠がある場合）
- 意外な周辺事業
- 有報原文だけでは断定できない事項の扱い

例: 4091 日本酸素ホールディングスの場合、単にガス品目を列挙するだけでなく、
「産業ガスを中核に、半導体・電子材料向け特殊ガス、医療用ガス、在宅医療、
ガス関連機器、溶断・溶接機器を扱い、家庭用品ではサーモスも傘下に持つ」
という形で、主力と広がりが分かる文章を目指す。
...
会社概要ボックス内では、株たん由来の短文概要とAI整理を役割分担して表示する。

```text
1. stocks.business_summary があれば「短文概要」として表示
   → AI整理の有無にかかわらず残す
2. business_overviews.overview があれば「AI整理」として表示（整理済み事業概要）
   → 「AI整理（有報 YYYY-MM-DD 時点）」の鮮度注記を必ず付ける
3. AI整理がなければ business_description の冒頭を表示
