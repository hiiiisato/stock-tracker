...
_CATEGORY_RULES = [
    ...
    ("split",         ("株式分割",), +1),
    # エクイティは人事より先に評価する（RS＝譲渡制限付株式としての自己株式処分は
    # 役員・執行役員を含むが本質はエクイティなので、人事へ誤入させない）。
    ("equity",        ("新株予約権", "第三者割当", "公募増資", "自己株式の処分", "自己株式処分",
                       "譲渡制限付株式", "譲渡制限株式", "株式給付信託", "株式交付", "株式報酬",
                       "株式売出", "ストックオプション", "新株式発行", "行使価額"), 0),
    ("personnel",     ("人事", "役員異動", "役員の異動", "役員等の異動", "役員体制", "新役員体制",
                       "役員選任", "役員就任", "役員候補者", "役員退任", "役員の担当", "役員担当",
                       "役員の体制", "役員等の体制", "役員新体制", "代表取締役", "執行役員", "監査役",
                       "取締役の選任", "取締役選任", "取締役候補者", "取締役人事", "取締役の異動",
                       "役員の選任", "役員その他", "取締役及び執行役員", "取締役および執行役員",
                       "機構改革", "組織改編",
                       "組織及び人事", "組織変更のお知らせ"), 0),
