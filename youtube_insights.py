...
- 週次: 週1実行（GHA・土曜朝）。CHANNELS を全巡回 → youtube_weekly → /youtube・LINE週報
- 日次: 平日19:00 JST（GHA）。DAILY_CHANNELS（大引け解説）を巡回 → youtube_daily →
  日次レポート(/daily)の資金フロー前に差し込む。手動: python youtube_insights.py --daily
- 手動実行: python youtube_insights.py [--daily] [--limit N] [--video VIDEO_ID]

【チャンネルの増減】CHANNELS / DAILY_CHANNELS を編集するだけ（ハンドルは youtube.com/@xxx のxxx部分）。
"""
...
CHANNELS: list[dict] = [
    ...
    {"handle": "fukugankeizai",    "name": "複眼経済塾（渡部清二）"},
    {"handle": "kabu.moriguchi",   "name": "森口亮・毎日チャート分析"},
...
MAX_PER_CHANNEL = 2      # 1チャンネルあたり最大何本（チャンネル増に伴い分散重視で2本に）
MAX_ANALYZE     = 12     # 1回の実行でGemini分析する最大本数（無料枠・GHA50分timeoutに配慮）
GEMINI_MODEL    = "gemini-2.5-flash"

# 日次モード（大引け後の相場解説を巡回・日次レポートに差し込む。平日19:00 JST実行）。
# 大引け解説を上げる少数チャンネルに絞る（週次の網羅巡回とは別枠）。
DAILY_CHANNELS: list[dict] = [
    {"handle": "iwaicosmosec",     "name": "岩井コスモ証券"},
    {"handle": "j-kabu-moriguchi", "name": "日本株速報（森口亮）"},
    {"handle": "NikkeiCNBC",       "name": "日経CNBC"},
]
DAILY_LOOKBACK_DAYS   = 1    # 当日分のみ（直近24時間）
DAILY_MAX_PER_CHANNEL = 2
DAILY_MAX_ANALYZE     = 6
...
ANALYZE_PROMPT = """この投資系YouTube動画を分析し、以下のJSONのみを出力してください（コードブロック不要）。
{
  ...
  "stocks": [
    {"name": "銘柄名", "code": "証券コード4桁(日本株のみ・不明なら空)", "view": "強気|弱気|中立",
     "reason": "**背景・材料・根拠のある理由**（決算・受注・新製品・規制・バリュエーション・
       業績見通し等。具体的な数字を含める）"}
  ],
  "stance": "発信者の相場観・スタンス（1文）",
  "actionable": "視聴者への具体的な示唆があれば（1文・なければ空）",
  "key_points": ["動画中の重要な具体的情報を5-8個。数字・固有名詞・根拠をそのまま保持する。
    例: '安川電機の1Q営業益はコンセンサスを約30%下回った（ロボット事業の失速）'
    例: 'ASMLが通期売上見通しを上方修正、4-6月純利益27%増'"]
}
【stocksに入れる基準（重要）】
- 「なぜそう動いた/そう見るのか」の背景・材料がある銘柄のみ入れる。
- 「〇〇が弱気、X%下落」のような**値動きだけで背景が無い言及は入れない**（使えない情報のため除外）。
- 発信者が理由・材料に触れず、単に株価が動いた事実だけを述べた銘柄は stocks から除外してよい。
- 指数・ETFは stocks に入れず themes/market へ。動画内で実際に言及された内容のみ（創作禁止）。"""
...
{{
  ...
  "hot_stocks": [{{"name": "銘柄名", "code": "コード", "note": "**材料・根拠を伴う**言及内容（1文）。値動きだけの銘柄は入れない"}}],
...
## 個別銘柄の見方
**背景・材料のある銘柄のみ**を挙げる。1銘柄1-2行: **銘柄名(コード)** — 強気/弱気と、
その根拠（決算・受注・新製品・規制・バリュエーション等）を具体的な数字とともに。
複数の発信者が言及した銘柄は「複数言及」と明示（注目度が高いシグナル）。
★除外基準（厳守）: 材料・根拠（決算/受注/新製品/規制/業績見通し/バリュエーション等）を伴わない
言及は載せない。以下は全て除外:
 - 「〇〇が弱気、X%下落」のような値動きだけの言及
 - 「Appleが最高値更新、週間+5%」のような理由の無い株価変動
 - 「AI関連株全体の下落につられて下げた」のような、その銘柄固有の材料が無いもの
背景・材料が薄い銘柄は、たとえ動画で言及されていても載せないこと（情報の質を優先）。
## 見方が分かれる点
あれば。なければこのセクションごと省略。
## 来週の注目点
...
DAILY_PROMPT = """あなたは投資情報誌の編集者です。以下は本日の株式投資系YouTube動画{n}本の分析結果
（各動画の要点・具体的ファクト・銘柄への見方）です。これらを**全て集約・整理して1本の日次レポート**に
仕上げてください。大引け後の個人投資家が「今日の相場で何が語られたか」を数分で掴めることが目的です。
...
{{
  "summary": "本日のマーケット状況と発信者たちの共通見解（3〜4文・結論ファースト）。**具体性を最優先**: 『半導体関連の一部』のような曖昧な括りは禁止。言及された具体的な銘柄名・指数の値動き(％)・材料を必ず織り込む（例: 『日経平均は+1.2%。アドバンテストがAI向けテスタ需要で+5%と牽引、一方○○は決算失望で-8%』）",
  "consensus": "強気/弱気の全体トーン（1文）",
  "hot_themes": [{{"theme": "複数動画または強い確信で言及されたテーマ", "note": "何が言われているか＋関連銘柄名を具体的に（1文）"}}],
  "hot_stocks": [{{"name": "銘柄名", "code": "コード", "note": "**材料・根拠＋できれば値動き(％)を伴う**具体的な言及内容（1文。例: 決算好調で+7%、通期上方修正）。値動きだけ/背景の無い銘柄は入れない"}}],
  "divergence": "発信者間で見方が分かれている論点があれば（1文・なければ空）",
  "report_md": "Markdown形式の日次レポート本文（後述の構成・500〜1000字）"
}}

report_md の構成（Markdown・## 見出し）:
## 今日の結論
2-3行。今日の相場で何が起き、明日に向けて何を見るべきか。
## マーケット概況
各発信者の見立てを統合。指数の具体的な動き・数字は保持する（例: 日経平均は本日+1.2%）。
## 注目テーマ
テーマごとに小見出し(###)を立て、「何が起きたか→発信者の見方→関連銘柄」。具体的な数字・事実を残す。
## 個別銘柄の見方
**背景・材料のある銘柄のみ**を挙げる。1銘柄1-2行: **銘柄名(コード)** — 強気/弱気と、
その根拠（決算・受注・新製品・規制・バリュエーション等）を具体的な数字とともに。
複数の発信者が言及した銘柄は「複数言及」と明示。
★除外基準（厳守）: 材料・根拠を伴わない値動きだけの言及は載せない。指数・ETFは個別銘柄に入れない。

厳守事項:
- 抽象論に逃げず、素材のkey_pointsにある具体的な数字・固有名詞・根拠を積極的に本文へ残す
- 素材に無い情報を創作しない。銘柄コードは素材にあるもののみ使用
- どの発信者の見方かが分かるように適宜「（日経CNBC）」等と出典を添える"""
...
def ensure_tables(cur) -> None:
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS youtube_daily (
            ...
            themes_json TEXT,             -- [{theme, note}]
            stocks_json TEXT,             -- [{name, code, note}]
            report_md   TEXT,             -- 日次レポート本文(Markdown・大引け解説を集約)
            created_at  DATETIME
        )
    """)
    # 旧スキーマからの移行（列が無ければ追加）
...
# セクター名など「個別銘柄でない」語（ETFに誤マッチしやすい）
_SECTOR_WORDS = {"銀行", "不動産", "証券", "保険", "半導体", "自動車", "鉄鋼", "商社",
                 "海運", "空運", "電機", "化学", "医薬", "小売", "食品", "建設",
...
def _resolve_code(cur, code: str, name: str) -> str:
    ...
    for cond, arg in [("name=%s", name), ("name LIKE %s", f"{name[:8]}%")]:
        if len(name) < 2:
            break
        cur.execute(f"SELECT code, COUNT(*) OVER() c FROM stocks WHERE is_active=1 AND {cond} LIMIT 1",
                    (arg,))
        row = cur.fetchone()
        if row and row[1] == 1:    # 一意に定まる時だけ採用
            return row[0]
    return ""
...
def aggregate_daily(cur, client, day: date) -> bool:
    """日次チャンネルの直近分析済み動画を集約して日次サマリーを生成・保存。"""
    handles = tuple(ch["handle"] for ch in DAILY_CHANNELS)
    ph = ",".join(["%s"] * len(handles))
    cur.execute(f"""
        SELECT channel, title, market, stance, themes_json, stocks_json, notes_json
        FROM youtube_videos
        WHERE status='analyzed' AND handle IN ({ph}) AND published >= %s
        ORDER BY published DESC
    """, (*handles, datetime.now() - timedelta(days=DAILY_LOOKBACK_DAYS)))
    rows = cur.fetchall()
    if not rows:
        print("  日次集約対象なし")
        return False
    ...
    cur.execute("""
        INSERT INTO youtube_daily
            (report_date, n_videos, summary, consensus, divergence, themes_json, stocks_json,
             report_md, created_at)
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,NOW())
        ON DUPLICATE KEY UPDATE
            n_videos=VALUES(n_videos), summary=VALUES(summary), consensus=VALUES(consensus),
...
def _crawl_and_analyze(cur, conn, client, channels: list[dict], lookback_days: int,
                       max_per: int, max_analyze: int, verbose: bool) -> dict:
    """指定チャンネル群の直近動画を収集 → 未分析のみGemini分析。週次/日次で共用。"""
    ...
    for ch in channels:
        ...
        picked = 0
        for v in vids:
            if v["published"] < since or picked >= max_per:
                continue
...
def run_daily(max_analyze: int = DAILY_MAX_ANALYZE, verbose: bool = True) -> dict:
    """大引け後の相場解説チャンネルを巡回 → 日次サマリー生成（daily_reportに差し込む）。
    LINEは日次レポート側（20:30確定版）で送るため、ここでは通知しない。"""
    if not GEMINI_API_KEY:
        print("GEMINI_API_KEY未設定のためスキップ")
        return {"videos_found": 0, "analyzed": 0, "failed": 0, "skipped": 0}
    conn = get_conn()
    ...
    conn.close()
    if verbose:
        print(f"完了(日次): 発見{stats['videos_found']} 分析{stats['analyzed']}"
              f" 失敗{stats['failed']} 除外{stats['skipped']}")
    return stats
