...
WEEKLY_PROMPT = """以下は今週の株式投資系YouTube動画{n}本の分析結果です。横断して整理し、以下のJSONのみを出力してください。
{videos}
    ...
    cur.execute("""
        INSERT INTO youtube_videos
            (video_id, handle, channel, title, published, status,
             market, stance, actionable, themes_json, stocks_json, analyzed_at)
        VALUES (%s,%s,%s,%s,%s,'analyzed',%s,%s,%s,%s,%s,NOW())
        ON DUPLICATE KEY UPDATE
            status='analyzed', market=VALUES(market), stance=VALUES(stance),
            actionable=VALUES(actionable), themes_json=VALUES(themes_json),
            stocks_json=VALUES(stocks_json), analyzed_at=NOW()
...
def aggregate_weekly(cur, client, week_end: date) -> bool:
    ...
    cur.execute("""
        SELECT channel, title, market, stance, themes_json, stocks_json
        FROM youtube_videos
    ...
    if not rows:
        ...
        return False
    digest = []
    for ch, title, market, stance, tj, sj in rows:
    ...
    cur.execute("""
        INSERT INTO youtube_weekly
            (week_end, n_videos, summary, consensus, divergence, themes_json, stocks_json, created_at)
        ...
        ON DUPLICATE KEY UPDATE
            ...
            stocks_json=VALUES(stocks_json), created_at=NOW()
    """, (week_end, len(digest),
