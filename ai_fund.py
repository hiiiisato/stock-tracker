...
13. 検証済みプレイブック（docs/trade_strategy_research.md）に従う。特に「深押しからの逆張り」と
    「出来高急増のみを根拠にした初動買い」は実測で期待値マイナスのため取らない
14. 利益保全: 取得後の高値が+25%を超えた銘柄が高値から-10%まで陥落したら機械的に売却（規律を判断より優先）"""
...
PLAYBOOK = """【主力】A:52週高値ブレイク型 — 出来高(5日/25日比)1.3倍以上を伴い52週高値を更新・更新間近の銘柄。
  実測: 3459取引 平均+9.2% 中央値+3.5% 勝率54%（下記2段階エグジット併用時）
【副力】C:トレンド押し目反発型 — 200日線が上向きで株価がその上、RSI42未満まで調整→5日で+3%以上の反発を確認。
  実測: 2554取引 平均+6.4%。平常時は+3%程度だが、市場急落直後のリバウンド局面では平均+20%超
【禁止】52週高値から-40%超の深押し逆張り、出来高急増のみを根拠にした初動買い
  （全シグナル検証で期待値マイナス。勝った例だけ見ると魅力的に見える選択バイアスに注意）
【エグジット規律・2段階】買値-12%で損切り。+25%到達後は「高値-10%」のトレールに切替。
  利が乗る前のMA25割れ売りはwhipsawで期待値を毀損する（実測: 平均+2.0%まで劣化）
【天井の平均像】起点から60〜70日・RSI75前後。天井後9〜10日でMA25割れ（そこで売ると6〜7割捕捉）
【時間】想定保有60〜130営業日。60日で+10%に届かなければ入替を検討"""
...
def _candidates(cur, exclude: set[str]) -> list[dict]:
    ...
    views = [
        ...
        ("breakout", sel + base_from + """
            AND p.dev_high52w >= -1 AND p.vol20_ratio >= 1.3 AND p.rsi14 < 82
...
EDGE_VIEWS = {
    ...
    "breakout": "dev_high52w >= -1 AND vol20_ratio >= 1.3 AND rsi14 < 82",
...
def decide() -> int:
    ...
    for p in positions:
        if p["pnl_pct"] <= LOSSCUT_PCT:
            ...
            continue
        # 利益保全: 取得後高値が+25%超 → 高値-10%まで陥落したら売り（プレイブックのトレール切替）
        cur.execute("""
        ...
        if hi and hi >= p["avg_cost"] * 1.25 and p["close"] <= hi * 0.90:
            hi_pct = (hi / p["avg_cost"] - 1) * 100
            p["_force_reason"] = (f"[利益保全規律] 取得後高値+{hi_pct:.0f}%到達後、高値から-10%まで陥落"
