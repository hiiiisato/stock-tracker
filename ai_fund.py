...
14. 利益保全: 取得後の高値が+{PROFIT_ARM_PCT:.0f}%を超えた銘柄が高値から-{PROFIT_TRAIL_PCT:.0f}%まで陥落したら機械的に売却
    （規律を判断より優先。テーマ相場を長く乗るため、含み益は最大{PROFIT_TRAIL_PCT:.0f}%返してもトレンド継続中は保有を続ける）
...
【エグジット規律・2段階】買値-12%で損切り。+25%到達後は「高値-15%」のトレールに切替えて利を伸ばす。
  トレール-10%→-15%への変更で長期テーマ相場の捕捉率が53%→59%に改善（半導体勝者139銘柄で実測）。
  利が乗る前のMA25割れ売りはwhipsawで期待値を毀損する（実測: 平均+2.0%まで劣化）
...
STRATEGIES = {
    ...
    "V": {"name": "割安×成長",       "entry": "理論株価比1.25倍以上×営業増益×需給が崩れていない",
          "edge": "（理論株価・スクリーニングのバックテスト由来）", "role": "中期の柱。モメンタム消失時の受け皿"},
...
_STRAT_TAG_ORDER = [("event", "E"), ("breakout", "A"), ("rs_leader", "R"), ("theme_leader", "T"),
                    ("dip", "C"), ("value_growth", "V"), ("laggard", "L"), ("momentum", "R")]
...
def execute_orders() -> int:
    ...
    for oid, code, side, budget, shares, reason, thesis, decided, style, catalyst, strategy in orders:
        if decided >= today:
        ...
        if side == "sell":
            ...
            cur.execute("""
                INSERT INTO ai_fund_trades (code, side, shares, price, fee, trade_date, decided_date,
                                            reason, buy_reason, pnl, pnl_pct, hold_days, style, strategy, created_at)
                VALUES (%s,'sell',%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW())
            ...
            cur.execute("""
                INSERT INTO ai_fund_trades (code, side, shares, price, fee, trade_date, decided_date, reason, style, catalyst, strategy, created_at)
                VALUES (%s,'buy',%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW())
            """, (code, n, open_px, round(fee), today, decided, reason, style or "通常", catalyst, strategy))
...
[タグ] momentum=上昇モメンタム / dip=上昇トレンド中の押し目 / breakout=52週高値ブレイク(主力A) / rs_leader=6ヶ月相対強度上位×高値圏(主力R・実測平均+12%) / theme_leader=強テーマの主役(副力T・平均は高いが当たり外れ大) / value_growth=割安×成長 / event=上方修正・増配 / laggard=資金流入テーマ内の出遅れ(先回り向き・リターンは主役の半分と実測) / bench=昨日までの控え銘柄
...
{{"market_view": "市況の見立て(2文以内)",
 ...
 "buys": [{{"code": "XXXX", "budget": 1200000, "style": "通常", "strategy": "A",
...
def decide() -> int:
    ...
    for p in positions:
        ...
        if (hi and hi >= p["avg_cost"] * (1 + PROFIT_ARM_PCT / 100)
                and p["close"] <= hi * (1 - PROFIT_TRAIL_PCT / 100)):
            hi_pct = (hi / p["avg_cost"] - 1) * 100
            p["_force_reason"] = (f"[利益保全規律] 取得後高値+{hi_pct:.0f}%到達後、高値から-{PROFIT_TRAIL_PCT:.0f}%まで陥落"
                                  f"（現在{p['pnl_pct']:+.1f}%）。検証済み2段階エグジットに従いトレール利確。")
            forced_sells.append(p)
    ...
    for b0 in valid_buys:
        cur.execute("""
            INSERT INTO ai_fund_orders (code, side, budget, reason, catalyst, thesis, style, strategy, decided_date, created_at)
            VALUES (%s,'buy',%s,%s,%s,%s,%s,%s,%s,NOW())
        """, (b0["code"], b0["budget"], b0["reason"], b0.get("catalyst", ""), b0["thesis"],
              b0.get("style", "通常"), b0.get("strategy"), today))
        n_orders += 1
