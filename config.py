...
# 成長率ブレンドの逓減係数。将来の成長率を「1年目=会社予想 → 以降は過去3年CAGR
# （長期平均）へ decay^(n-1) で回帰」させる。1社の一時的な勢いを永久には仮定せず、
# 遠い将来ほど長期平均に寄せる（個人投資家の安全側の発想）。
#   g_n = 長期 + (1年目 - 長期) × THEO_GROWTH_DECAY^(n-1)
THEO_GROWTH_DECAY  = 0.60
# 長期成長率の上限（±/年, 小数）。過去3年CAGRが高くても、5年も持続する前提は
# 非現実的なため長期側だけ抑える（1年目の会社予想は THEO_GROWTH_CAP まで許容）。
THEO_LONGRUN_CAP   = 0.20
