...
**取引カレンダーの正本（2026-09-22障害後に再設計）**: `trading_calendar` はJ-Quants公式の
休日区分（`master.update_trading_calendar`）を過去〜契約範囲まで採用し、それより先（無料枠の
窓外）はJPX公式サイトの「休業日一覧」（`jpx_calendar.py`。翌々年末まで掲載）で平日と祝日を埋める。
HolDiv 0・3=休場、1・2=営業日（3=祝日取引ありの日で株式は休場。旧実装はこれを営業日として誤登録
しており、2025-09-23等24日分の原因だった）。daily_run.pyの休場判定（`_calendar_status`）は
「営業日／休場／未登録」の3状態を返し、未登録を営業日扱いしない（未登録のまま処理を進めると
「営業日なのに価格0件」を誤検知する＝2026-09-22の障害）。価格データの有無で`trading_calendar`を
上書きする処理は廃止した（Yahooは休場日にもstale値の「幽霊行」を返すことがあり、公式休場日を
Yahooで覆すのは危険なため）。`master.calendar_lookahead_days()` が今日から連続何日分カレンダーが
埋まっているかを数え、`CALENDAR_MIN_LOOKAHEAD_DAYS`(180日)を割り込んだらdaily_runを失敗させる
（JPXページの構造変更で未来分の取り込みが止まったことに早期に気づくため）。
...
- `theme_master.py` — **統一テーママスタ（みんかぶを正とする。2026-07再設計）**。
  minkabu.jp の全テーマ(~780)と**銘柄ごとの編集済み関連度(0-100)**を同期（kabutanと同じ
  ミンカブ・ジ・インフォノイド運営でテーマ体系は同一だが、関連度はみんかぶのみ公開）。
  tier: 関連度70+=3(コア)/50+=2(関連)。**集計(money_flow等)は tier>=2 のみ**。
  指数構成・IPO年度等の非テーマは除外フィルタ。みんかぶから消えて21日で自動archive。
  `theme_members.manual_lock`(pin3/pin2/pin1/exclude)は同期に常に勝つ（手動メンテ用）。
...
|---|---|
| `/` | ホーム（指数・注目銘柄） |
| `/stock/<code>` | 銘柄詳細（チャート・業績・理論株価・メモ）。四半期業績は単独3か月値をQ1〜Q4表示し、最新四半期の増収増益等の評価・加速/減速・前年同期比・利益率・会社予想進捗を結論カードで表示。グラフ横軸はQ×決算期の2段階層、比較表は指標×四半期の横持ち（スマホは最新列へ自動スクロール） |
...
| `/watchlist` | 複数リスト分類、共通メモ、終値/PER/PBR/配当利回り、3か月ミニチャート、上値/下値リマインダーと到達フラグ。閲覧は公開、変更は `WATCHLIST_PASSCODE`（未設定時は `PORTFOLIO_PASSCODE`）とCSRF検証で保護 |
